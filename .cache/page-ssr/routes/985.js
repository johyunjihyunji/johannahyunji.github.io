...
*/
var aa=__webpack_require__(1725),ca=__webpack_require__(9982);function p(a){for(var b="https://reactjs.org/docs/error-decoder.html?invariant="+a,c=1;c<arguments.length;c++)b+="&args[]="+encodeURIComponent(arguments[c]);return"Minified React error #"+a+"; visit "+b+" for the full message or use the non-minified dev environment for full errors and additional helpful warnings."}var da=new Set,ea={};function fa(a,b){ha(a,b);ha(a+"Capture",b)}
...
function router_createRouter(init) {
  ...
  async function startRedirectNavigation(request, redirect, isNavigation, _temp2) {
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace === true || redirect.response.headers.has("X-Remix-Replace") ? router_Action.Replace : router_Action.Push;
...
async function convertDataStrategyResultToDataResult(dataStrategyResult) {
  ...
  if (type === ResultType.error) {
    ...
    return {
      type: ResultType.error,
      error: result,
      statusCode: router_isRouteErrorResponse(result) ? result.status : undefined
    };
  }
...
function dist_useResolvedPath(to, _temp2) {
  ...
  let routePathnamesJson = JSON.stringify(getResolveToMatches(matches, future.v7_relativeSplatPath));
  return index_js_.useMemo(() => router_resolveTo(to, JSON.parse(routePathnamesJson), locationPathname, relative === "path"), [to, routePathnamesJson, locationPathname, relative]);
}
